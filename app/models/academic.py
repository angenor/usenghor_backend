...
class Program(Base, UUIDMixin, TimestampMixin):
    ...
    teaching_methods_html: Mapped[str | None] = mapped_column(Text)
    teaching_methods_md: Mapped[str | None] = mapped_column(Text)
    objectives: Mapped[list[str] | None] = mapped_column(JSONB)
    target_audience: Mapped[list[str] | None] = mapped_column(JSONB)
    format_html: Mapped[str | None] = mapped_column(Text)
    format_md: Mapped[str | None] = mapped_column(Text)
    ...
    campus_rel: Mapped["Campus | None"] = relationship(
        "Campus",
        primaryjoin="Program.campus_external_id == foreign(Campus.id)",
        viewonly=True,
        lazy="selectin",
    )
